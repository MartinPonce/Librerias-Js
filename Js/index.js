...
let establecerDatos = function () {
    ...
    //OPERADOR TERNARIO
    let plazoMensual = document.getElementById('mensual').checked   //LA CONDICION
    let plazoAnual = document.getElementById('anual').checked

    if ( plazoMensual === true ) {    // EJECUCION EN CASO QUE SE CUMPLA
        this.plazo = plazo
    } else if ( plazoAnual === true ) {  // EJECUCION EN CASO QUE SE CUMPLA
        this.plazo = plazo * 12
    } else {
        alert('No seleccionaste ningún tipo de plazo')  // ¨ELSE¨ SI NO SE CUMPLE
    }
 // const plazos = (plazoMensual.plazo && plazoAnual.plazo) ? true : false //OPERADOR LOGICO "AND"
 // mostramos el mensaje
 // plazos ? alert("Seleccionaste el plazo correctamente") : alert("No seleccionaste ningún tipo de plazo")
...
let [a, b, c, d] = datos   POSICIONAL
Let [,, a, b]              OMITO LAS PRIMERAS POSICIONES

console.log(a, b, c)   POSICIONALES
console.log(...datos) SPREAD DE ARRAYS   */  
...
for (let i = 0; i < localStorage.lenght; i++) {   //OPERADOR ++ EJ. SUGAR SYNTAX
...
localStorage.clear('') //elimina toda la info
sessionStorage.clear('') //elimina toda la info
...
function calcularTotalPrestamo () {
    let totalPrestamo = 0
    for ( let i = 0; i < totalPagos; i++ ) { //OPERADOR ++ EJ. SUGAR SYNTAX
...
function simularPrestamo () {
    ...
    //header de mi tabla
    for ( let j = 0; j < columnas.length; j++ ) {    //OPERADOR ++ EJ. SUGAR SYNTAX
